...
MODULE_AUTHOR("Siran Shen and Li-Wei Tseng");
...
typedef struct pid_list_node {
	pid_t pid;
	pid_list_node_t* next;
} pid_list_node_t;

typedef struct pid_list {
	pid_list_node_t* first;
	unsigned size;
} pid_list_t;

typedef struct ticket_list_node {
	unsigned ticket;
	ticket_list_node_t* next;
} ticket_list_node_t;

typedef struct ticket_list {
	ticket_list_node_t* first;
	unsigned size;
} ticket_list_t;
*/
...
typedef struct osprd_info {
	...
	unsigned *exited_tickets;
	int exited_tickets_cnt;

	pid_t current_pid; // current running process
...
static void osprd_setup(osprd_info_t *d)
{
	...
	d->reader_cnt = d->writer_cnt = d->exited_tickets_cnt = 0;
	d->exited_tickets = (unsigned *) kmalloc(1024 * sizeof(unsigned), GFP_ATOMIC);
	d->current_pid = -1;
}
